...
async def start_interview(session_id, db: AsyncSession = Depends(get_db)):
    return await start_interview_controller(session_id, db)


@interview_route.post("/submit", response_model=AnswerResponse)
async def submit_answer(answer_req: AnswerRequest, db: AsyncSession = Depends(get_db)):
    return await submit_answer_controller(answer_req, db)
...
async def end_interview(session_id: str, db: AsyncSession = Depends(get_db)):
    return await end_interview_controller(session_id, db)


@interview_route.get("/report/{session_id}", response_model=ReportResponse)
async def report(session_id: str, db: AsyncSession = Depends(get_db)):
    return await generate_report_controller(session_id, db)
