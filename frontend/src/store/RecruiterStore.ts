...
export const useRecruiterStore = create<any>()(
    (set) => ({
        ...
        getRecruiterInvites: async () => {
            set({ isLoading: true })
            try {
                const resp = await axiosInstance.get("/recruiter/invites")
                set({ getRecruiterInvitesRes: resp.data, isLoading: false })
            } catch (error: any) {
                set({ getRecruiterInvitesRes: null, isLoading: false })
                toast.error(error.response?.data?.detail || 'internal server error')
                console.error('error in getRecruiterInvitesRes dashboard', error)
            }
        },
    })
)